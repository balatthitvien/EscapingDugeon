...
@onready var tile_map_layer: TileMapLayer = get_node_or_null("TileMapLayer") as TileMapLayer
...
func _ready() -> void:
	...
	if tile_map_layer != null:
		...
		push_warning("map_5: Không tìm thấy TileMapLayer để set bounds.")
...
func setup_player_death_handler() -> void:
	...
	if is_two_player_mode():
		...
		if players.is_empty():
			push_warning("map_5: Không tìm thấy Player nào để bắt signal died.")
			return
		...
		print("map_5: Đã kết nối died cho cả 2 player.")
		return
...
func apply_player_spawn_point() -> void:
	...
	if is_two_player_mode():
		...
		for p in players:
			...
			print("map_5: Đã đưa ", p.name, " tới spawn point: ", spawn_name, " tại ", p.global_position)
	else:
		if PlayerManager.player != null:
			...
			print("map_5: Đã đưa Player tới spawn point: ", spawn_name, " tại ", spawn_point.global_position)
